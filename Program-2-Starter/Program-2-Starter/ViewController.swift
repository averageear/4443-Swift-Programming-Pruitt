...
class ViewController: UIViewController , ColorTableViewControllerDelegate{
    ...
    @IBOutlet weak var prevImage5: UIImageView!
    ...
    func imageHistory()->Void{
        let myImage = UIImage(named: "doesThisHaveToHaveAName?")
        var prevImageArray: [UIImageView] = [prevImage, prevImage2, prevImage3, prevImage4, prevImage5]
        let index = imageHarry.count
        var k = 0
        //give all images the same background image...doesn't work
        /*for i in 0..<6 {
            prevImageArray[i].image = myImage
        }*/
        
        //find and set the imageViews' backgroundColor
        if (index == 0){
           println("error setting color because array is empty")
        }else if(index < 5){
            for i in 0..<index{
                prevImageArray[i].backgroundColor = imageHarry[i]
            }
        }else if(index >= 5){
            for (var j = imageHarry.count-5; j < imageHarry.count; j++){
                prevImageArray[k].backgroundColor = imageHarry[j]
                k++
            }
        }
    ...
    func ColorTableViewChooseColor(controller: ColorTableViewController, chosenColor color: String){
        ...
        var myImage = UIImage(named: "doesThisHaveToHaveAName")
        prevImage.image = myImage
        prevImage2.image = myImage
        prevImage3.image = myImage
        prevImage4.image = myImage
        prevImage5.image = myImage
        ...
        HSVLabel.text = ("HSV:\t\t \(hsv.H)\t \(hsv.S)\t \(hsv.V)")
        HSLLabel.text = ("HSL:\t\t \(hsl.H)\t \(hsl.S)\t \(hsl.L)")
        
        imageHarry.append(labelColor)
        imageHistory()
